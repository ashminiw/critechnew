...
function Modal() {
	...
	const uploadPost = async () => {
		...
		//1.create a post and ad to fb 'posts' collection
		...
		const docRef = db.collection("posts").doc("hi").add({
			username: user.email,
			caption: captionRef.current.value,
			// timestamp: serverTimestamp()
		})
		//show in console and docRef.id is the post ID
		console.log("New doc added with ID", "hi");

		//upload to firestore with postID
		const imageRef = storage.ref(`posts/hi/image`);


		imageRef.putString(selectedFile, "data_url").then(async snapshot => {
			const downloadURL = await imageRef.getDownloadURL();
			var ashref = db.collection("posts").doc("hi")
